...
状态：已实施，随 `v2.3.0-beta.2` 提供测试。核对日期：2026-09-10。设计基线：本地 `main`，提交 `58b34a3`。以下为历史方案记录；实际用法见 `configuration.md` 和 `library.md`，当前代码入口见 [架构与维护入口](architecture.md)。

实施调整：复用已安装的 `golang.org/x/net/http/httpproxy` 在实例创建时快照环境变量规则，避免标准库进程级缓存与重建时的环境变化；环境测试无需子进程。另修复了代理握手挂起时实例关闭不能立即取消的问题。依赖升级到当前兼容版本，Go 更新为 1.26.8，req 为 3.61.2。
