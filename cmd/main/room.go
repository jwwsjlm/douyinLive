...
func NewRoomManager(logger *appLogger, unknown bool, cookie string, roomCookies map[string]string, tikHubKey string, pollInterval time.Duration, notifyInterval time.Duration) *RoomManager {
	if logger == nil {
...
func (rm *RoomManager) GetOrCreateRoom(roomID string, cookieOverride string) *Room {
	...
	room = NewRoom(roomID, rm.logger, rm.unknown, cookie, rm.tikHubKey, rm.pollInterval, rm.notifyInterval, func() {
		rm.roomsMu.Lock()
		delete(rm.rooms, key)
		rm.roomsMu.Unlock()
...
func NewRoom(id string, logger *appLogger, unknown bool, cookie string, tikHubKey string, pollInterval time.Duration, notifyInterval time.Duration, onClose func()) *Room {
	if logger == nil {
		logger = newAppLogger(nil)
	}
	return &Room{
