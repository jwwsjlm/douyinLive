...
func NewApp(ctx context.Context, config *Config, logger *appLogger) (*App, error) {
	...
	roomManager := NewRoomManager(logger, config.Unknown, config.Cookie.Douyin, config.Cookie.Rooms, config.TikHub.Key, config.Monitor.PollInterval, config.Monitor.NotifyInterval)
	return &App{
		ctx:         ctx,
		logger:      logger,
		config:      config,
