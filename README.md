...
--tikhub-key string  TikHub API Key，用于在线生成 WebSocket xb 签名
--version            输出版本和构建来源
```
...
监听直播间 WebSocket 消息需要 TikHub API Key，用于在线生成 `xb` 签名。库模式推荐使用 `NewDouyinLiveWithTikHub` 创建实例：

```go
dl, err := douyinlive.NewDouyinLiveWithTikHub(roomID, log.Default(), cookie, tikHubKey)
```

如果你的项目使用 `log/slog`，可以直接用 `NewDouyinLiveWithSlogAndTikHub` 创建实例，日志会保留结构化级别和字段：
...
func main() {
	...
	cookie := ""
	// TikHub API Key，用于在线生成 WebSocket xb 签名
	tikHubKey := "YOUR_TIKHUB_KEY"
...
func main() {
	roomID := "516466932480"
	tikHubKey := "YOUR_TIKHUB_KEY"
	dl, err := douyinlive.NewDouyinLiveWithTikHub(roomID, log.Default(), "", tikHubKey)
	if err != nil {
		log.Fatal(err)
	}
